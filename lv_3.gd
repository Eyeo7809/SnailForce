...
func _on_fall_check_body_entered(body: Node2D) -> void:
	if $Protag.gems == 2:
		get_tree().change_scene_to_file("res://demo_over.tscn")
	else:
...
func _on_hive_body_entered(body: Node2D) -> void:
	if body.is_in_group("Protag"):
		body.SPEED = G.speed * 0.5
		print(str(body.SPEED))


func _on_hive_body_exited(body: Node2D) -> void:
	if body.is_in_group("Protag"):
		body.SPEED = G.speed
		print(str(body.SPEED))


func _on_hive_2_body_entered(body: Node2D) -> void:
	if body.is_in_group("Protag"):
		body.SPEED = G.speed * 0.5
		print(str(body.SPEED))
...
func _on_hive_2_body_exited(body: Node2D) -> void:
	if body.is_in_group("Protag"):
		body.SPEED = G.speed
		print(str(body.SPEED))


func _on_hive_tutorial_body_entered(body: Node2D) -> void:
	$HiveTutorial/HiveTutLab.visible = true


func _on_hive_tutorial_body_exited(body: Node2D) -> void:
	$HiveTutorial.queue_free()
